    ...
    public class SourcingRepository : ISourcingRepository
    {
        ...
        public async Task<bool> InsertRFQ(RFQProjectModel rFQProjects, List<RFQModel> rfqData)
        {
            try
            {
                ...
                {
                    string RFQProjectsQuery = @"INSERT INTO RFQProjects (ProjectName, Customer, QuotationCode, NoItems, RequestDate, RequiredDate, StdTAT, Status, HasPrices) " +
                                  "VALUES (@ProjectName, @Customer, @QuotationCode, @NoItems, @RequestDate, @RequiredDate, @StdTAT, @Status, @HasPrices)";
        ...
        public async Task<IEnumerable<dynamic>> RFQQuery(string partNumber, string tableName)
        {
            ...
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                ...
                {
                    "RFQ" => $"SELECT * FROM {tableName} WHERE ProjectName = @partNumber AND Remarks = 'FOR SOURCING'",
                    _ => $"SELECT DISTINCT i.Id, i.ProjectName, i.Customer, i.QuotationCode, i.NoItems, i.RequestDate, i.RequiredDate ,i.Status " +
        ...
        public async Task<bool> HasPrices(string projectName)
        {
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                string query = @"UPDATE RFQProjects SET HasPrices = 1 WHERE ProjectName = @ProjectName";

                int result = await conn.ExecuteAsync(query, new { ProjectName = projectName });
        ...
        public async Task<IEnumerable<RFQModel>> GetRFQPartNumbers(ProjectAndQuotation RFQ)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(_connectionString))
        ...
        public async Task<bool> SaveSupplierAndComments(PartData data)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(_connectionString))
                {
                    string query = @"UPDATE RFQ SET SuggestedSupplier = @SuggestedSupplier, Comments = @Comments WHERE CustomerPartNumber = @PartNumber AND ProjectName = @ProjectName";
